...
export const signup = (email, nickname, password, confirmPassword) => {
  return axios.post(`${SERVER_BASE_URL}/signup`, { email, nickname, password, confirmPassword }).then(res => {
    return res.data;
  });
}

export const login = (email, password) => {
  return axios.post(`${SERVER_BASE_URL}/login`, { email, password }).then(res => {
    return res.data;
  })
}

export const fetchAllSections = () => {
  return axios.get(`${SERVER_BASE_URL}/sections`, {
    headers: {
...
export const fetchPosts = section => {
  return axios.get(`${SERVER_BASE_URL}/${section}/posts`, {
    headers: {
      Authorization: 'Bearer ' + localStorage.getItem('token')
    }
  }).then(res => {
